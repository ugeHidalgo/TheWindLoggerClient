...
export class ImportSessionMaterialsHelper {
  ...
  import(data): Observable<SessionMaterial[]> {
    ...
    for (var f=1; f<data.length; f++)
    {
      ...
      console.log('Imported session material for session :' + sessionMaterial.session + ' and material ' + sessionMaterial.material);
    }

    console.log('Imported session materials data from excell, sending to server to create.');
    return me.sessionMaterialsService.importSessionMaterials(sessionMaterials);
  }
};
